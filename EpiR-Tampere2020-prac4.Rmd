...
## 9. Age-standardised incidence rates

We use popEpi package to calculate the age-standardised incidence by calendar year in the whole population of Finland. Data sibr includes the dates of diagnosis and the ages at diagnosis. 

```{r, eval=TRUE,echo=TRUE,warnings=FALSE}
library(popEpi)
data(sibr)

coh <- transform(sibr, dg_y = cal.yr(dg_date) )
coh$agegroup <- cut(coh$dg_age, breaks=c(seq(0,85,5),Inf), right=F)
coh$dg_y <- floor(coh$dg_y)
```

Calculate the number of cases by agegroup and calendar year

```{r, eval=TRUE,echo=TRUE,warnings=FALSE}
tab <- stat.table( index = list(agegroup,year=dg_y), 
            contents = list(count() ), 
            margins = F, data = coh)
```

Tabulate the mid-year population in Finland in 5-year age groups
```{r, eval=TRUE,echo=TRUE,warnings=FALSE}
pop <- meanpop_fi[sex==1 & year%in%1993:2012 ]
pop$agegroup <- cut(pop$agegroup, breaks=c(seq(0,85,5),Inf), right=F)

tab.pop <- stat.table( index = list(agegroup,year), 
                   contents = list(meanpop=sum(meanpop) ), 
                   margins = F, data = pop)
```

Combine the numbers of cases and person-years

```{r, eval=TRUE,echo=TRUE,warnings=FALSE}
#combine the numbers of cases and person-years
tab <- merge(data.frame(as.table(tab[1,,])), 
             data.frame(as.table(tab.pop[1,,])),by=c("agegroup","year"))
names(tab)[3:4] <- c("obs","pyrs")
tab$year <- as.numeric(as.character(tab$year))
```

Choose a standard population: population of Finland in 2012
```{r, eval=TRUE,echo=TRUE,warnings=FALSE}
own_weights <- tab[tab$year==2012,]
own_weights <- own_weights[order(own_weights$agegroup),]
```

rate() function in popEpi package can be used to calculate the rates with confidence intervals
```{r, eval=TRUE,echo=TRUE,warnings=FALSE}
tab1 <- rate(tab, obs="obs", pyrs="pyrs", adjust="agegroup", 
             weights="world_1966_18of5", print="year")
#OR equivalently: 
#tab1 <- rate(tab, obs="obs", pyrs="pyrs", adjust="agegroup", 
#            weights=stdpop18$world, print="year")
tab2 <- rate(tab, obs="obs", pyrs="pyrs", adjust="agegroup", 
             weights=own_weights$pyrs, print="year")
```

# Age-standardised incidence rate in population aged 25+

Restrict to 25+ population by setting the weights of the youngest age groups to 0.
```{r, eval=TRUE,echo=TRUE,warnings=FALSE,fig.width=10,fig.height=7}
own_weights$pyrs25 <- own_weights$pyrs
own_weights$pyrs25[1:5] <- 0

#25+ population of World in 1966 as another standard
stdpop18$world25 <- stdpop18$world
stdpop18$world25[1:5] <- 0
```

Age-standardised incidence is calculated using the weights where the weights of the youngest age groups are 0. 
```{r, eval=TRUE,echo=TRUE,warnings=FALSE,fig.width=10,fig.height=7}
tab1_25 <- rate(tab, obs="obs", pyrs="pyrs", adjust="agegroup", weights=stdpop18$world25, print="year")
tab2_25 <- rate(tab, obs="obs", pyrs="pyrs", adjust="agegroup", weights=own_weights$pyrs25, print="year")
```

Non-standardised incidence has to be calculated by excluding the youngest age groups.
```{r, eval=TRUE,echo=TRUE,warnings=FALSE,fig.width=10,fig.height=7}
tab0_25 <- rate(tab[as.numeric(tab$agegroup)>5,], obs="obs", pyrs="pyrs", print="year")
```

Compare age-standardised rates using different standard populations
```{r, eval=TRUE,echo=TRUE,warnings=FALSE,fig.width=10,fig.height=7}

plot(as.numeric(tab1$year),tab1$rate*100000, type="l",ylim=c(0, 50),
     ylab="Incidence per 100 000 person years")
lines(as.numeric(tab1$year),tab1$rate.adj*100000, col="red")
lines(as.numeric(tab2$year),tab2$rate.adj*100000, col="green")

lines(as.numeric(tab0_25$year),tab0_25$rate*100000,lty=2)
lines(as.numeric(tab1_25$year),tab1_25$rate.adj*100000, col="red",lty=2)
lines(as.numeric(tab2_25$year),tab2_25$rate.adj*100000, col="green",lty=2)

legend("topleft",c("Non-standardised incidence",
                   "Age-standardised incidence (World 1966)",
                   "Age-standardised incidence (Finland 2012)",
                   "Non-standardised incidence in 25+ population",
                   "Age-standardised incidence in 25+ population (World 1966)",
                   "Age-standardised incidence in 25+ population (Finland 2012)"),
       col=c("black","green","red","black","green","red"),lty=c(1,1,1,2,2,2))
```
