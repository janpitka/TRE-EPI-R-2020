...
```{r p5_2, eval=TRUE,echo=TRUE,warnings=FALSE}
head( thoro )
thoro$birthdat[1:6]
...
## 9. sir() function in popEpi package


```{r, eval=TRUE,echo=TRUE,warnings=FALSE}
library(popEpi)
gmortDK$ref.rate <- gmortDK$rt/1000
m <- sir( coh.data = thL.apd, coh.obs = 'd', coh.pyrs = 'y', 
          ref.data = gmortDK, ref.rate = 'ref.rate', 
          adjust = c('agr', 'pgr' ,'sex'), 
          print = 'contrast', conf.type = "wald")
m
```


Estimate SMR over calendar time using splines

```{r, eval=F,echo=F,warnings=FALSE}
m <- sirspline( 
  coh.data = thL.apd, coh.obs = 'd', coh.pyrs = 'y', 
  ref.data = gmortDK, ref.rate = 'ref.rate', 
  adjust = c('agr', 'pgr' ,'sex'), print = 'contrast', 
  spline = 'pgr', knots=5)

year <- m$spline.seq.A
est <- m$spline.est.A
plot(year,est[est$i=="Other",2], type="l", col="blue", 
     ylim=c(0,50), ylab="SMR", xlab="Year")
lines(year,est[est$i=="Thoro",2], col="red")
#95% CIs
for(k in 3:4){
  lines(year,est[est$i=="Other",k], lty=2, col="blue")
  lines(year,est[est$i=="Thoro",k], lty=2, col="red")
} 
abline(h=1)
legend("topright",c("Thoro","Other"),col=c("red","blue"),lty=1)
```
...
```{r, eval=TRUE,echo=TRUE,warnings=FALSE}
mT <- sirspline( 
  coh.data = thL.apd[thL.apd$contrast=="Thoro",], coh.obs = 'd', coh.pyrs = 'y', 
  ref.data = gmortDK, ref.rate = 'ref.rate', 
  adjust = c('agr', 'pgr' ,'sex'), spline = 'pgr', knots=5)

mO <- sirspline( 
  coh.data = thL.apd[thL.apd$contrast=="Other",], coh.obs = 'd', coh.pyrs = 'y', 
  ref.data = gmortDK, ref.rate = 'ref.rate', 
  adjust = c('agr', 'pgr' ,'sex'), spline = 'pgr', knots=5)
```

Point estimates with 95% confidence intervals for the two groups

```{r, eval=TRUE,echo=TRUE,warnings=FALSE}
plot(mT, conf.int = F, ylim=c(0,30))
lines(mT, conf.int = T, lty=2)
lines(mO, conf.int = F,col="red")
lines(mO, conf.int = T,col="red", lty=2)
legend("topright",c("Thoro","Other"),col=c("black","red"),lty=1)
```


## 10. Use data.table to obtain a table of observed and expected deaths cross-classified by age and period:
